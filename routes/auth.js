...
router.post('/changepassword', CheckLogin, ChangePasswordValidator, validatedResult, async function (req, res, next) {
    ...
    // req.user in CheckLogin is set to an array `getUser` from GetUserById.
    let userId = req.user[0] ? req.user[0]._id : req.user._id; 
    let result = await userController.ChangePassword(userId, oldpassword, newpassword);
    if (result) {
        res.send({ message: "Doi mat khau thanh cong" });
    } else {
        res.status(400).send({ message: "Mat khau cu khong dung hoac loi xac thuc" });
    }
})
